...
async function reqSafe(url, options = {}) {
  try {
    ...
    console.error('请求异常:', e); // 新增错误日志，便于调试
    return {};
  }
}
...
async function category(tid, pg, _, ext = {}) {
  ...
  return JSON.stringify({
    ...
    page: +pg,
    pagecount: 9999,
    limit: 30, // 修复：pageSize是30，limit对应30更合理
    total: 999999
  });
}
...
async function detail(id) {
  ...
  // 核心修复：按每集的name拼接播放地址，而非固定取第0项
  if (vod.episodelist?.length) {
    // 遍历每一集，用当前集的name作为标识，保证集数唯一
    vod.vod_play_url = vod.episodelist.map(ep => {
      const epName = ep.name || `第${ep.nid}集`; // 兜底：如果name为空，用nid生成集数名称
...
async function play(_, id) {
  ...
  if (Array.isArray(res.data?.list)) {
    for (const item of res.data.list) {
      if (item.resolutionName && item.url) { // 过滤空地址
